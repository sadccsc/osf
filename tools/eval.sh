...
while [ "$cdate" -le $enddate ]; do
	...
	overwrite=${item[9]}
        ...
        mapdir=$rootdir/maps
        args="$datadir $mapdir $model $predictorvar $predictordomain $predictandinstitution $predictandvar $predictanddomain $mos $cdate $basetime $mask $overwrite $targetseason"
        echo
        echo "*********************************"
        echo calling pycpt_singlefcst.py with $args


        python $scriptdir/eval.py $args
    done
